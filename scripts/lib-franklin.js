 ...
 */
export async function fetchPlaceholders(accessKey = '',prefix = "default") {
  window.placeholders = window.placeholders || {};
  const loaded = window.placeholders[`${accessKey}`];
  if (!loaded) {
      ...
      try {
        fetch(`${prefix === "default" ? "" : prefix}/placeholders.json`)
          ...
          .then((json) => {
            ...
            window.placeholders[accessKey] = placeholders;
            resolve();
          });
      } catch (error) {
        // error loading placeholders
        window.placeholders[accessKey] = {};
        reject();
      }
    });
  }
