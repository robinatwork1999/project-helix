...
export async function decorateIcons(element) {
  ...
  icons.forEach((span) => {
    ...
    const closestTags = ["a", "p"];
    [...closestTags].forEach((ele) => {
      if (span.closest(ele)) {
        span.closest(ele).classList.add(`content__icon`);
      }
    });
...
export function createOptimizedPicture(
  ...
  breakpoints = [
    ...
    { media: "screen and (min-width: 768px) and (max-width: 1023px)", width: "700", height: "300" },
    { media: "screen and (min-width: 1024px)", width: "2000", height: "500" }]
) {
  ...
  breakpoints.forEach((br) => {
    ...
    source.setAttribute("srcset", `${pathname}?width=${br.width}&height=${br.height}&format=webply&optimize=medium`);
